...
defineSupportCode(({ Given, When, Then }) => {
    ...
    Then(/^a list of items is displayed$/, () => {
        browser
            .element('#translation-list-container')
            .waitForVisible(DEFAULT_WAIT_TIME)
    })

    Then(/^pagination is displayed$/, () => {
        browser
            .element('ul.data-table-pager--buttons')
            .waitForVisible(DEFAULT_WAIT_TIME)
        browser
            .element('li.data-table-pager--page-info')
            .waitForVisible(DEFAULT_WAIT_TIME)
        browser
            .element('li.data-table-pager--previous-page')
            .waitForVisible(DEFAULT_WAIT_TIME)
        browser
            .element('li.data-table-pager--next-page')
            .waitForVisible(DEFAULT_WAIT_TIME)
    })

    Then(/^first object item of the list is open for translation$/, () => {
        const firstCardGrids = browser
            .elements('#translation-list-container div[class^=MuiPaperroot]')
            .value[0].elements('div[class^=MuiGridcontainer]').value
    ...
    Then(/^save action for open property should be displayed$/, () => {
        browser
            .element(
                '#translation-list-container div[class^=MuiPaperroot] button'
            )
            .waitForVisible(DEFAULT_WAIT_TIME)
    })
    ...
    // **************************************************************
    Then(
        /^I see a translation for (.+) with an existing translation (.+) for (.+)$/,
        (object, translation, property) => {
            const selectedObject = browser
                .element('#select-object-id')
                .element('<input>')
                .getAttribute('value')
            const selectedPropertyTranslation = browser
                .elements(
                    '#translation-list-container div[class^=MuiPaperroot]'
                )
                .element(`#${property}`)
